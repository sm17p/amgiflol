<script lang="ts">
  import { createMessageHandler, messageBus } from "@/lib/core/MessageBus";
  ...
  let isCollapsed = $derived(
    userCollapsed !== null ? userCollapsed : !expanded
  );
  ...
  function updatePerformanceData() {
    ...
    performanceData.elements = document.querySelectorAll("*").length;
  ...
  onMount(() => {
    if (showPerformance) {
      performanceInterval = setInterval(updatePerformanceData, 1000);
    }
    ...
    const mouseUpdateInterval = setInterval(updateCurrentElement, 100);
...
{#if import.meta.env.DEV && uiStore.debugToolbar.isVisible}
  <div class="relative z-10 text-white">
    <div class="flex justify-between text-lg px-6 py-3">
      <span class="bg-neutral-800 rounded-lg p-2">{mouse.x} x {mouse.y}</span>
      <span class="bg-neutral-800 rounded-lg p-2"
