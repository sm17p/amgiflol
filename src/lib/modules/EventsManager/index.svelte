<script lang="ts">
  ...
  function setupMessageHandlers() {
    const toggleHandler = createMessageHandler(
      "EXTENSION_TOGGLE",
      (payload: any, message) => {
        if (!message.source.content && payload.isActive !== uiStore.isActive) {
          void uiStore.toggleActive(message.source);
        }
      }
    );

    const zoomHandler = createMessageHandler("ZOOM_CHANGE", (payload: any) => {
...
<svelte:document
  onmousemove={uiStore.isActive ? handleMouseMove : undefined}
  onkeydown={uiStore.isActive ? handleKeyDown : undefined}
  onkeyup={uiStore.isActive ? handleKeyUp : undefined}
  onmouseover={uiStore.isActive ? handleMouseOver : undefined}
/>
<svelte:window
  bind:innerWidth={metadataStore.window.innerWidth}
  bind:innerHeight={metadataStore.window.innerHeight}
  onresize={uiStore.isActive ? handleWindowResize : undefined}
