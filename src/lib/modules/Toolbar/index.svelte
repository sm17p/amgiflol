<script lang="ts">
  ...
  const disableForFirefox = !import.meta.env.DEV && import.meta.env.FIREFOX;
  ...
  function handleKeyDown(
    ...
    if (event.key === "7") {
      designMode(true);
    } else if (event.key === "9" || (event.altKey && event.code === "Digit9")) {
      capture(true);
    }
  }
        ...
        >
          ...
          {#if trackers.current}
            <ToolbarAction
              ...
              label={`${
                trackers.current.isLocked ? "Unlock" : "Lock"
              } Inspector`}
              shortcut="@"
            >
          ...
          <ToolbarAction
            ...
            shortcut="8"
          >
            <PanelRightOpen absoluteStrokeWidth class="size-4 sm:size-6" />
          </ToolbarAction>
          <div
            role="separator"
            class="bg-neutral-200 -my-1 mx-0.5 sm:mx-1 w-0.5 self-stretch"
